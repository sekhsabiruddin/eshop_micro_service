...
interface ShopCardProps {
  shop: {
    ...
    address?: string;
    rating?: number;
    followers?: { length: number }[]; // Fixed typo from "folowers"
  };
}
  ...
  return (
    <div className="w-full rounded-md cursor-pointer bg-white border border-gray-200">
      ...
      <div className="relative flex justify-center -mt-8">
        <div className="w-16 h-16 rounded-full border-4 border-white overflow-hidden shadow bg-white">
          <Image
            src={
              shop.avatar ||
              "https://ik.imagekit.io/fzoxwtey/avatar/6_N7eMmuAvl.png?updatedAt=1742269698784"
            }
            alt={shop.name}
            width={64}
            height={64}
      ...
      <div className="mt-4 text-center pb-4">
        <Link
          href={`/shop/${shop.id}`}
          className="inline-flex items-center text-sm text-blue-600 font-medium hover:underline hover:text-blue-700"
        >
          Visit Shop
